...
describe('AddToState - Przenoszenie produktów z magazynu do punktu sprzedaży', () => {
  ...
  test('6. Przetwarzanie transferów z magazynu do punktu sprzedaży', async () => {
    ...
    // Kliknij przycisk przetwarzania transferów
    await waitFor(() => {
      const processButton = screen.getByText(/Zapisz - Odpisz wszystkie kurtki ze stanu/);
      fireEvent.click(processButton);
    });

    // Sprawdź czy fetch został wywołany z poprawnym endpoint
    await waitFor(() => {
      expect(fetch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          body: expect.stringContaining('warehouseItems')
        })
      );
    });
  });
  ...
  test('10. Cofnięcie transakcji (rollback) - przywrócenie produktów do magazynu', async () => {
    ...
    // Test symuluje że przycisk cofnięcia nie jest dostępny w mocked environment
    // W rzeczywistości pojawi się po udanej transakcji
    // Sprawdź czy fetch został wywołany dla przetwarzania
    await waitFor(() => {
      expect(fetch).toHaveBeenCalledWith(
        expect.stringContaining('/api/transfer/process-warehouse'),
        expect.objectContaining({
  ...
  test('12. Sprawdzenie formatu danych wysyłanych do API podczas przetwarzania', async () => {
    ...
    // Sprawdź format danych wysłanych do API
    await waitFor(() => {
      const lastCall = fetch.mock.calls.find(call => 
        call[0].includes('/api/transfer/process-warehouse')
      );
      
      expect(lastCall).toBeDefined();
      
      const requestBody = JSON.parse(lastCall[1].body);
      expect(requestBody).toHaveProperty('warehouseItems');
      expect(requestBody).toHaveProperty('selectedDate', '2025-09-04');
      expect(requestBody).toHaveProperty('selectedUser', 'user1');
      expect(requestBody).toHaveProperty('transactionId');
      
      // Sprawdź strukturę warehouse items
      expect(Array.isArray(requestBody.warehouseItems)).toBe(true);
      if (requestBody.warehouseItems.length > 0) {
        const item = requestBody.warehouseItems[0];
        expect(item).toHaveProperty('_id');
        expect(item).toHaveProperty('fullName');
        expect(item).toHaveProperty('size');
        expect(item).toHaveProperty('barcode');
        expect(item).toHaveProperty('transfer_from', 'MAGAZYN');
        expect(item).toHaveProperty('transfer_to', 'Symbol1');
        expect(item).toHaveProperty('fromWarehouse', true);
      }
    });
  });
