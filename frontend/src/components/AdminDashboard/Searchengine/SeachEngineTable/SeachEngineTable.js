...
const SeachEngineTable = () => {
    ...
    const [selectedDate, setSelectedDate] = useState(new Date().toISOString().split('T')[0]); // State for selected print date - default to today
    const [resolvedStateData, setResolvedStateData] = useState([]); // State for resolved state data with selling points
    ...
    const fetchProducts = async () => {
        try {
            ...
            const stateDataFromAPI = stateResponse.data.map((stateItem) => ({
                ...stateItem,
                fullName: stateItem.fullName, // Use fullName directly from state
                size: stateItem.size, // Use size directly from state
                sellingPoint: stateItem.symbol, // 🚨 POPRAWKA: Używaj symbol zamiast sellingPoint
            }));

            // Extract unique symbols from state data
            const uniqueSymbols = [...new Set(stateDataFromAPI.map((item) => item.symbol))];
            setSymbols(uniqueSymbols);
            
            // Save resolved state data to state for inventory calculations
            setResolvedStateData(stateDataFromAPI);
            ...
            stateDataFromAPI.forEach((stateItem) => {
    ...
    const generatePrintPage2HTML = (selectedDate = '') => {
        ...
        const calculateInventorySums = () => {
            ...
            allProductsStateData.forEach(stateItem => {
                ...
                // Mapuj punkt sprzedaży na nasze nazwy - POPRAWIONE MAPOWANIE!
                switch(sellingPoint) {
                    case 'KRUPÓWKI':
                    case 'K':
                        sums.krupowki += quantity;
                        break;
                    case 'TATA':
                    case 'T':
                        sums.tata += quantity;
                    ...
                    case 'PARZYGNAT':
                    case 'P':
                        sums.parzygnat += quantity;
                        break;
                    case 'SKRZAT':
                    case 'S':
                        sums.skrzat += quantity;
                        break;
                }
            });
        ...
        const inventorySums = calculateInventorySums();
        const totalSum = inventorySums.krupowki + inventorySums.tata + inventorySums.most + inventorySums.parzygnat + inventorySums.skrzat;
        ...
        // Filtruj kamizelki męskie z kożucha - LIMIT 8
        const menVests = printTableData.filter(row => {
            const productData = products.find(p => p.fullName === row[2]);
            if (!productData) return false;
            
            return isMenVest(productData, row[2]);
        }).slice(0, 8); // LIMIT 8 kamizelek męskich

        // Filtruj kamizelki damskie z kożucha - LIMIT 10
        const womenVests = printTableData.filter(row => {
            const productData = products.find(p => p.fullName === row[2]);
            if (!productData) return false;
            
            return isWomenVest(productData, row[2]);
        }).slice(0, 10); // LIMIT 10 kamizelek damskich

        // Filtruj kamizelki JR z lisa - LIMIT 10
        const jrVests = printTableData.filter(row => {
            const productData = products.find(p => p.fullName === row[2]);
            if (!productData) return false;
            
            return isJRVest(productData, row[2]);
        }).slice(0, 10); // LIMIT 10 kamizelek JR z lisa
        ...
        <body>
            ...
            <div class="section section-right">
                ...
                <!-- Sekcja kamizelek męskich pod dziecięcymi -->
                <div style="margin-top: 15px;">
                    <div class="section-title">Kamizelki męskie z kożucha</div>
                    <table>
                        <thead>
                            <tr>
                                <th class="product-name" style="background-color: #ffffff !important;">Nazwa</th>
                                ...
                                <th class="size-cell">6XL/64</th>
                                <th class="size-cell size-7xl">7XL/66</th>
                                <th class="size-cell">8XL/68</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${menVests.map((row, rowIndex) => {
                                const bgColor = row[1] || '#ffffff';
                                const isDark = isColorDark(bgColor);
                                return `
                                <tr style="
                ...
                <!-- Sekcja kamizelek damskich pod kamielek męskimi -->
                <div style="margin-top: 15px;">
                    <div class="section-title">Kamizelki damskie z kożucha</div>
                    <table>
                        <thead>
                            <tr>
                                <th class="product-name" style="background-color: #ffffff !important;">Nazwa</th>
                ...
                <!-- Sekcja kamizelek JR z lisa pod kamielek damskich z kożucha -->
                <div style="margin-top: 15px;">
                    <div class="section-title">Kamizelki JR z lisa</div>
                    <table>
                        <thead>
                            <tr>
                                <th class="product-name" style="background-color: #ffffff !important;">Nazwa</th>
                ...
                <!-- Sekcja z wybraną datą pod kamielek JR -->
                ${selectedDate ? `
                ...
                <!-- Tabelka z danymi o stanach magazynowych -->
                <div style="margin-top: 20px; text-align: center;">
                    <table style="
                        margin: 0 auto; 
                        border-collapse: collapse; 
                    ...
                    ">
                        <tbody>
                            <tr>
                                <td style="border: 1px solid #000000; padding: 5px 10px; background-color: #f8f9fa;">S</td>
                                <td style="border: 1px solid #000000; padding: 5px 10px; text-align: left;">
                                    ilość sztuk na stanie u Skrzata: <span style="margin-left: 10px; font-weight: bold;">${inventorySums.skrzat}</span>
                                </td>
                            </tr>
                            <tr>
                                <td style="border: 1px solid #000000; padding: 5px 10px; background-color: #f8f9fa;">T</td>
                                <td style="border: 1px solid #000000; padding: 5px 10px; text-align: left;">
                                    ilość sztuk na stanie u Taty: <span style="margin-left: 10px; font-weight: bold;">${inventorySums.tata}</span>
                                </td>
                            </tr>
                            <tr>
                                <td style="border: 1px solid #000000; padding: 5px 10px; background-color: #f8f9fa;">M</td>
                                <td style="border: 1px solid #000000; padding: 5px 10px; text-align: left;">
                                    ilość sztuk na stanie pod Mostem: <span style="margin-left: 10px; font-weight: bold;">${inventorySums.most}</span>
                                </td>
                            </tr>
                            <tr>
                                <td style="border: 1px solid #000000; padding: 5px 10px; background-color: #f8f9fa;">K</td>
                                <td style="border: 1px solid #000000; padding: 5px 10px; text-align: left;">
                                    ilość sztuk na stanie na Krupówkach: <span style="margin-left: 10px; font-weight: bold;">${inventorySums.krupowki}</span>
                                </td>
                            </tr>
                            <tr>
                                <td style="border: 1px solid #000000; padding: 5px 10px; background-color: #f8f9fa;">P</td>
                                <td style="border: 1px solid #000000; padding: 5px 10px; text-align: left;">
                                    ilość sztuk na stanie u Parzygnata: <span style="margin-left: 10px; font-weight: bold;">${inventorySums.parzygnat}</span>
                                </td>
                            </tr>
                            <tr style="background-color: #e8f5e8; font-weight: bold;">
                                <td colspan="2" style="border: 2px solid #000000; padding: 8px 10px; text-align: center; font-size: 16px;">
                                    RAZEM ŁĄCZNA ILOŚĆ SZTUK W ZAKOPANEM: <span style="margin-left: 10px; font-weight: bold; color: #d63384;">${totalSum}</span>
                                </td>
                            </tr>
                        </tbody>
                    </table>
    ...
    const handlePrintPage2 = () => {
        ...
        // Pobierz aktualne dane stanów magazynowych
        const getStateData = async () => {
            try {
                const response = await axios.get('/api/goods/states');
                return response.data;
            } catch (error) {
                console.error('Błąd pobierania danych stanów:', error);
                return [];
            }
        };
        ...
        const printContent = generatePrintPage2HTML(selectedDate);
        const printWindow = window.open('', '_blank');
        printWindow.document.write(printContent);
        printWindow.document.close();
